...
// Helper function to convert string to int for older C++ standards
int stringToInt(const string& str) {
    return atoi(str.c_str());
}

vector<vector<int> > readMatrix(const string& filename, int& size) {
    // Convert string to c-string for older compilers
...
void multiplyMPI(const vector<vector<int> >& A, const vector<vector<int> >& B,
    ...
    // Master process flattens the matrices
    if (rank == 0) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                flat_A[i * n + j] = A[i][j];
    ...
    // Master process reconstructs the matrix
    if (rank == 0) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                C[i][j] = flat_C[i * n + j];
...
int main(int argc, char** argv) {
    ...
    // Allocate matrices for non-master processes
    if (rank != 0) {
